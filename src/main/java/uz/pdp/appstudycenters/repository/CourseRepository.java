...
public interface CourseRepository extends JpaRepository<Course, Integer> {
    ...
    @Query(value = "select co.id, co.description, co.name, co.company_id from course co join company on company.id=co.company_id join address on company.address_id=address.id join district on district.id=address.district_id join region district.region_id=region.id where region.name=:region and co.name=:name", nativeQuery = true)

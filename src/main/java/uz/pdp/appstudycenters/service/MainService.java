...
public class MainService {
    ...
    public List<Course> searchByRegionName(String region) {

        Optional<List<Course>> courseByRegionName = courseRepository.getCourseByRegionName(region);
        if (!courseByRegionName.isPresent())
            return new ArrayList<>();
        List<Course> courses = courseByRegionName.get();
        return courses;
    }

    public List<Course> searchByCourseCategory(String name) {

        Optional<List<Course>> courseByCourseCategoryName = courseRepository.getCourseByCourseCategoryName(name);
        if (!courseByCourseCategoryName.isPresent())
            return new ArrayList<>();
        List<Course> courses = courseByCourseCategoryName.get();
        return courses;
    }
}
